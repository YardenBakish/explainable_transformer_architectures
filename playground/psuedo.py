...
#FORWARD
x = patchEmbed(x)                          #             (B,196,192)
x = torch.cat((cls_tokens, x), dim=1)      # [x,cls]     (B,197,192)
x = Add([x, pos_embed])                    # x+pos_embed (B,197,192) + (B,197,192) =  (B,197,192)
...

#BACKWARDS - starting right after positional_embed + patch_embed
...
attribution, positional_encoding_attribution   = Add.backward(attribution)               #(B,197,192) , (B,197,192)
attribution                                    = attribution[:, 1:, :]                   #removing cls (B,196,192)
attribution_pixels                             = patchEmbed.relprop(attribution)         # (B,3,196,(16,16)) - 196 patches, 16x16 each
attribution_pixels                             = attribution_pixels.reshape(B,3,244,244) # (B,3,244,244)
attribution_pixels                             = attribution_pixels.sum(dim=1)

#NEW VERSION - BACKWARDS
# option 1 - normalize 
'''