...
class BertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if self.position_embedding_type == "relative_key" or self.position_embedding_type == "relative_key_query":
            warnings.warn("relative_key or relative_key_query position embedding type is NOT tested with LXT!")
            ...
            if self.position_embedding_type == "relative_key":
                ...
                attention_scores = lf.add2(attention_scores, relative_position_scores.detach())
            elif self.position_embedding_type == "relative_key_query":
                relative_position_scores_query = torch.einsum("bhld,lrd->bhlr", query_layer, positional_embedding)
                relative_position_scores_key = torch.einsum("bhrd,lrd->bhlr", key_layer, positional_embedding)
                attention_scores = lf.add2(attention_scores, (relative_position_scores_query + relative_position_scores_key).detach())

        attention_scores = lf.mul2(attention_scores, 1 / math.sqrt(self.attention_head_size))  
...
class BertLayer(nn.Module):
    ...
    def forward(
        ...
        if self.is_decoder and encoder_hidden_states is not None:
            ...
            outputs = lf.add2(outputs, cross_attention_outputs[1:-1])  # add cross attentions if we output attention weights
