...
attnlrp = Composite({
    ...
    ReluAttention: rules.IdentityRule,
...
#YARDEN
def apply_rotary_pos_emb(q, k, cos, sin,cos_K,sin_K, detachPE, position_ids=None, unsqueeze_dim=0):
    ...
    if detachPE:
        ...
        if cos_K is not None:
            k_embed = lf.add2(lf.mul2(k, cos_K), lf.mul2(rotate_half(k), sin_K)) #### <---- LXT
        else:
...
class LlamaMLP(nn.Module):
    def __init__(self, config):
        ...
        self.silu = nn.SiLU(inplace=False) #### <---- LXT
        self.proj_silu_mul = ProjSiluMultiplication() #### <---- LXT

    def forward(self, x):
        if self.config.pretraining_tp > 1:
            raise NotImplementedError("Pretraining TP > 1 is not supported in LXT right now. Please open GitHub issue.")
        else:
            # down_proj = self.down_proj(self.act_fn(self.gate_proj(x)) * self.up_proj(x))
            down_proj = self.down_proj(self.proj_silu_mul(self.silu(self.gate_proj(x)), self.up_proj(x))) #### <---- LXT
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: Optional[int] = None, attn_activation: Optional[nn.Module]= nn.Softmax(dim=-1)):
        super().__init__()
        self.config = config
        ...
        self.softmax = attn_activation #### <---- LXT
    ...
    def forward(
        ...
        **kwargs,
        ...
        if position_embeddings is None:
            ...
            if sep_heads:
                cos, cos_K, sin, sin_K = position_embeddings
            else:
                cos, sin = position_embeddings
        #print(detachPE)
        query_states, key_states = apply_rotary_pos_emb(query_states, key_states, cos, sin,cos_K,sin_K,detachPE)
        ...
        attn_weights = lf.mul2(lf.matmul(query_states, key_states.transpose(2, 3)), 1/math.sqrt(self.head_dim)) #### <---- LXT

        if attention_mask is not None:  # no matter the length, we just slice it
            causal_mask = attention_mask[:, :, :, : key_states.shape[-2]]
            # attn_weights = attn_weights + causal_mask
            attn_weights = lf.add2(attn_weights, causal_mask) #### <---- LXT

        # upcast attention to fp32
        #attn_weights = nn.functional.softmax(attn_weights, dim=-1, dtype=torch.float32).to(query_states.dtype)
        #attn_weights = nn.functional.dropout(attn_weights, p=self.attention_dropout, training=self.training)
        #attn_output = torch.matmul(attn_weights, value_states)
        attn_weights = self.softmax(attn_weights.float()).to(query_states.dtype) #### <---- LXT
        attn_output = self.attn_value_matmul(attn_weights, value_states) #### <---- LXT
...
def get_attention_layer(sequence_length, op):

    LLAMA_ATTENTION_CLASSES = {
        "baseline": LlamaAttention,
        "baseline_full": LlamaAttention,

        "attn_act_relu": partial(LlamaAttention, attn_activation=ReluAttention(n=sequence_length)),
        "attn_act_sigmoid": partial(LlamaAttention, attn_activation=SigmoidAttention(n=sequence_length)),

    }

    return LLAMA_ATTENTION_CLASSES[op]
...
class LlamaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states = lf.add2(residual, hidden_states) #### <---- LXT
...
class LlamaForSequenceClassification(LlamaPreTrainedModel):
    def __init__(self, config, attn_layer="baseline", sequence_length = 512):
        super().__init__(config)
...
class LlamaForTokenClassification(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        sequence_output = outputs[0]
        # sequence_output = self.dropout(sequence_output) ### <---- LXT
