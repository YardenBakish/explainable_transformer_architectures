...
def get_args_parser():
    ...
    parser.add_argument('--eval-coeff', type=float, default=1.5)
    # * Random Erase params
...
def main(args):
    ...
    data_loader_val = torch.utils.data.DataLoader(
        ...
        batch_size=int(args.eval_coeff * args.batch_size),
        num_workers=args.num_workers,
        pin_memory=args.pin_mem,
        drop_last=False
    )
    ...
    if args.finetune:
        ...
        for k in ['head.weight', 'head.bias', 'head_dist.weight', 'patch_embed.proj.weight', 'patch_embed.proj.bias',  'head_dist.bias']:
        ...
        print(num_current_extra_tokens)
        registers = None
        if ('registers' in args.variant) and args.resume == '' and 'registers' not in args.finetune:
          print("made it here")
          num_extra_tokens = 1
          num_registers = args.model_components["num_registers"]
     

        if ('registers' in args.variant) and args.resume == '' and 'registers' in args.finetune:
          
          num_extra_tokens = pos_embed_checkpoint.shape[-2] - num_patches 
          num_registers = args.model_components["num_registers"]
          old_registers = checkpoint_model['registers']
        ...
        if (num_current_extra_tokens > num_extra_tokens):
          registers = torch.full((1, num_registers, checkpoint_model['pos_embed'].shape[-1]), 0.1)
          new_pos_embed = torch.cat((extra_tokens, registers, pos_tokens), dim=1)
          print("here")
        elif (num_current_extra_tokens < num_extra_tokens):
          extra_tokens = extra_tokens[:,:num_registers+1,:]
          new_registers = old_registers[:,:num_registers,:]
          checkpoint_model['registers'] = new_registers
          
          print(extra_tokens.shape)
