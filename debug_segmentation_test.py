...
def eval_batch(image, labels, evaluator, index):
    ...
    if args1.method    == 'custom_lrp':
        Res1 = lrp1.generate_LRP(image.cuda(), method="custom_lrp",prop_rules = args1.prop_rules, cp_rule = args1.cp_rule).reshape(14, 14).unsqueeze(0).unsqueeze(0) 
        Res2 = lrp2.generate_LRP(image.cuda(), method="custom_lrp", prop_rules = args2.prop_rules, cp_rule = args2.cp_rule).reshape(14, 14).unsqueeze(0).unsqueeze(0) 
    ...
    #print(conc_imgs)
    print(f"basic pixelAccuracy: {pixAcc1}")
    print(f"relu pixelAccuracy: {pixAcc2}")
    print(f"basic mIOU: {mIOU1}")
    print(f"relu mIOU: {mIOU2}")
    ...
    print("\n")
    winner_pixAcc    = args1.variant if pixAcc1 > pixAcc2 else args2.variant
    winner_miou      = args1.variant if mIOU1 > mIOU2 else args2.variant
    winner_iou_fore  = args1.variant if IOU1[0] > IOU2[0] else args2.variant        
    ...
    return batch_correct1, batch_label1, batch_inter1, batch_union1, batch_ap1, batch_f11, pred1, target
